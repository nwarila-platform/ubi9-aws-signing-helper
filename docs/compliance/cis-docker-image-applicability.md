...
| CIS-Docker-4.2 | Image | Ensure that containers use only trusted base images. | `SATISFIED_REPO_CONTROL` | The final image is `FROM` Red Hat `ubi-micro`; both the `ubi-minimal` builder and the `ubi-micro` runtime are pinned by digest, and RPM inputs are installed through Red Hat repositories with GPG-checked microdnf. The helper source is fetched from the official AWS Roles Anywhere Credential Helper repository by release tag during the Dockerfile build. |
